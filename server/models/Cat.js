...
/* While Mongo is a schema-less database, meaning we can just store arbitrary objects in it,
   ...
   To create the schema we give it an object. Each top level key
   (like name, bedsOwned, and createdDate)
   defines a variable our objects can have. We then use individual
   objects to define the features and parameters
   of each one of those variables.

   For our name, we define a type of String. This means name is a string.
   Since mongo stores javascript objects, the type
   usually does not matter. However, this is something added by Mongoose to clarify what each
   variable should contain. We also give it the required option, which means every object
   in the cat database MUST have a name. We set the trim option to true, which will remove trailing
   and leading whitespace from the string. Finally we set the unique option to true.
   This means that there cannot be two cats with the same name.
